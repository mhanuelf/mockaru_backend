...
Pqr.getListPqrPerProduct = function(clientId, result) {
  ...
  sql.query(querys.PQR_LIST_PQR_PER_PRODUCT_TYPE, clientId, function(err, res) {
    if (err) {
...
Pqr.getListPqrPerClient = function(clientId, result) {
  ...
  var sqlQuery =
    "select TIPO_RADICACION, p.CONTRATO, DESCRIPCION_CASO, DESCRIPCION, f.IMPORTE_ORIGINAL, f.INTERESES_PAGADOS," +
...
Pqr.getListPqrPerProductType = function(productTypeId, result) {
  ...
  sql.query(querys.GET_PQR_BY_PRODUCT_TYPE, [productTypeId], function(
    err,
    res
  ) {
    if (err) {
